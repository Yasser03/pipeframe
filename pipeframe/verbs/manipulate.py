...
def _safe_eval_expression(df: pd.DataFrame, expr: str, column_name: str) -> pd.Series:
    ...
    if not isinstance(expr, str):
        raise PipeFrameTypeError(
            "Expression must be string", expected_type=str, got_type=type(expr)
        )
    ...
    for pattern in dangerous_patterns:
        if re.search(pattern, expr, re.IGNORECASE):
            raise PipeFrameExpressionError(
                expr, f"Expression contains potentially dangerous pattern: {pattern}"
            )
    ...
    try:
        ...
        raise PipeFrameExpressionError(expr, f"Failed to evaluate expression: {str(e)}") from e
...
def _define_impl(df: Union[DataFrame, pd.DataFrame], **kwargs: Any) -> DataFrame:
    ...
    for col_name, expr in kwargs.items():
        ...
        if not isinstance(col_name, str):
            raise PipeFrameTypeError(
                "Column name must be string", expected_type=str, got_type=type(col_name)
            )
        ...
        if isinstance(expr, str):
            ...
            try:
                ...
                raise PipeFrameExpressionError(
                    f"callable for '{col_name}'", f"Function failed: {str(e)}"
                ) from e
        else:
...
def _select_impl(df: Union[DataFrame, pd.DataFrame], *args: Any) -> DataFrame:
    ...
    for arg in args:
        if isinstance(arg, str):
            ...
            raise PipeFrameTypeError(
                f"Invalid selector type: {type(arg).__name__}. " "Use str, list, or callable"
            )
...
def filter_rows(df: Union[DataFrame, pd.DataFrame], condition: str) -> DataFrame:
    ...
    try:
        ...
        raise PipeFrameExpressionError(condition, f"Query failed: {str(e)}") from e
...
def _arrange_impl(df: Union[DataFrame, pd.DataFrame], *columns: Any, **kwargs: Any) -> DataFrame:
    ...
    for col in columns:
        if isinstance(col, tuple) and col[0] == "desc":
            ...
            raise PipeFrameTypeError(
                f"Invalid sort column type: {type(col).__name__}. " "Use str or desc(str)"
            )

    result = data.sort_values(by=sort_cols, ascending=sort_ascending, na_position=na_position)
...
def slice_rows(
    df: Union[DataFrame, pd.DataFrame], start: int = 0, stop: Optional[int] = None, step: int = 1
) -> DataFrame:
    """
    ...
    **kwargs : dict
    ...
    if isinstance(grouped_df, GroupBy):
        ...
        count_cols = [
            ...
            if isinstance(v, str) and v.split("(")[0] in ["count", "n", "size"]
        ]
...
def if_else(condition: str, true_value: Any, false_value: Any) -> Callable:
    ...
    def apply_if_else(df: DataFrame) -> pd.Series:
        ...
        try:
            ...
            raise PipeFrameExpressionError(condition, f"if_else condition failed: {str(e)}") from e

        result = pd.Series(np.where(mask, true_value, false_value), index=data.index)
        return result
    ...
    def apply_case_when(df: DataFrame) -> pd.Series:
        ...
        for condition, value in conditions:
            try:
                ...
                raise PipeFrameExpressionError(
                    condition, f"case_when condition failed: {str(e)}"
                ) from e

        result = pd.Series(
            np.select(conditions_list, choices_list, default=default_value), index=data.index
        )
        return result
