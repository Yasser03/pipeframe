    ...
    *required_cols : str
    ...
    def decorator(func: Callable) -> Callable:
        ...
        def wrapper(df: Union[DataFrame, pd.DataFrame], *args: Any, **kwargs: Any) -> Any:
            ...
            if missing:
                raise ValueError(
                    f"{func.__name__} requires columns: {required_cols}. " f"Missing: {missing}"
                )
...
def check_data_quality(
    df: DataFrame, checks: Optional[Dict[str, Callable]] = None
) -> Dict[str, bool]:
    """
...
def _peek_impl(
    df: DataFrame, n: int = 5, where: str = "head", message: Optional[str] = None
) -> DataFrame:
    """
